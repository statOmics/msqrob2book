...
[locally cached](#sec-caching)] from the github repository msqrob2data.

```{r}
library("BiocFileCache")
bfc <- BiocFileCache()
pepFile <- bfcrpath(bfc, "https://raw.githubusercontent.com/statOmics/msqrob2data/refs/heads/main/dda/francisellaFull/peptides.txt")
```
...
peps <- fread(pepFile, check.names = TRUE, integer64 = "double")
...
2. **Biological replicate effect**: the experiment involves biological
   ...
   $u_{biorep} \sim N(0,\sigma^{2,\text{b}})$. The use of random effects thus
...
```{r}
inference <- msqrobCollect(pe[["proteins"]],L)
head(inference)
```
...
targetProtein <- inference$feature[which.min(inference$adjPval)] #1
