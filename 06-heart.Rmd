...
**TODO** DISCUSS DO WE NEED TO USE RIDGE HERE? RESULTS REMAIN IN LINE. Slightly more features detected
...
There are `r length(sigNamesLeft)` proteins significantly
...
```{r heart_volcano_average, fig.cap=" "}
ggplot(inferenceAvg) +
  aes(x = logFC, y = -log10(pval), color = adjPval < 0.05) +
  geom_point() +
...
```{r heart_heatmap_average, fig.cap=" "}
...
There are `r length(sigNamesAvg)` proteins significantly
...
```{r heart_volcano_interaction, fig.cap=" "}
ggplot(inferenceInt) +
  aes(x = logFC, y = -log10(pval), color = adjPval < 0.05) +
  geom_point() +
