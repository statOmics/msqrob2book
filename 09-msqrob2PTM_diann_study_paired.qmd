# msqrob2PTM: DIA-NN case study - Paired phospho-enriched and non-enriched samples {#sec-msqrob2PTM-paired}
...
```{r data_path}
bfc <- BiocFileCache() # for usage with docker
...
We keep peptides that were observed at last 6 times out of the 
$n= 24$ samples, so that we can estimate the peptide characteristics. 
...
regardless of the modification, which is clearly illustrated for the top DA 
phospho-precursor! 

Any changes in the abundance of a phospho-peptidoform can thus be confounded 
with changes in protein abundance.

We therefore first assess differential abundance at the protein level for the 
non-enriched samples and will then propose an differential usage analysis where 
the precursor-level log2-FC is corrected for the protein-level log2-FC.  
...
We see for multiple top DA phospho-precursors, that the reason for DA seems to 
be associated with the similar changes in overall protein abundance. 
Interestingly for other phospho-precursors the abundance of the precursor 
changes between strains while the abundances at the protein-level remains stable 
between strains.
Hence, there is a stron need to correct the phospho-precursor level fold changes/
abundances for corresponding changes at the protein level.
...
We can explore the data for a protein to validate the statistical inference 
results. For example, let’s explore the normalised precursor abundance, usages 
