...
label effects. Indeed, for these designs run acts as a blocking
...
protein intensities that are measured within the same label will be
...
labeled experiments involving multiple, we also need to account for
...
derivative of $\rho$ is bounded. The most popular function for robust
...
The estimation is performed using iteratively reweighted least square,
where the weigths for each iteration are defined for each data point 
using the function : 
...
`msqrob2` also allows for ridge regression (when `ridge = TRUE`). In 
ridge regression, model parameters are estimated by minimising a 
penalised extension of the OLS loss:

$$
\sum\limits_{i = 1}^n \left(y_i - \mathbf{x}_i^T \boldsymbol{\beta}\right)^2 + \lambda||\boldsymbol{\beta}||^2
$$

Where $||\boldsymbol{\beta}||$ is the frobenius norm of
$\boldsymbol{\beta}$, i.e. $||\boldsymbol{\beta}|| =
\sqrt{\sum\limits_{j = 1}^p \beta_j^2}$. 

Due to the penalisation term $\lambda||\boldsymbol{\beta}||^2$ in the
loss function, the estimates of $\boldsymbol{\beta}$ will tend to be
pushed towards zero, especially for irrelevant covariates in
$\mathbf{X}$. In other words, the parameters are stabilised by 
reducing the variance of the estimation:

Ridge regression is particularly useful when dealing with near
covariates that contain similar information about the samples
(colinearity) or when there are multiple irrelevant covariates (soft
feature selection). Hence, ridge prevents overfitting by reducing the
the variance of estimation, as the variance-covariance matrix of the
estimation of $\beta$ is

$$
\hat{\sigma}^2_\beta = (\mathbf{X}^T\mathbf{X}+\lambda \mathbf{I})^{-1} \mathbf{X}^T\mathbf{X} (\mathbf{X}^T\mathbf{X}+\lambda \mathbf{I})^{-1} \hat{\sigma}_\epsilon^2
$$
while $\hat{\sigma}^{2, OLS}_\beta = \mathbf{X}^T\mathbf{X} 
\hat{\sigma}_\epsilon^2$. However, the penalty term in the loss
function also implies that the estimates of $\boldsymbol{\beta}$ will
be biased towards smaller values. This is known as the variance-bias
tradeoff. Another disadvantage of ridge is that it take more time to
estimate.
...
**TODO**: discuss/show link between ridge and mixed models? 
...
in [the section above](#sec-tmt_protein_model)^[Note that the inference
...
```
## Error in msqrobRefit(spikein, i = "ions", subset = oneHitProteins, formula = ~Condition + : could not find function "msqrobRefit"
```
